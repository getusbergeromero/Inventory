...
<body>
  ...
  filter: brightness(0.5);font-size: 25px; margin-left:-15;font-weight:bold;">Maintainance</h3>
   ...
   color:white; border-radius:10px;" data-bs-toggle="modal" data-bs-target="#staticBackdrop">
    ...
    <table class="table table-borderless" style="justify-content: center; height: 30%; width: 83%; margin-left:16%;background-color: whitesmoke; color: black; font-size:13px;">
        <thead class="thead-light">
            <tr>
                <th scope="col" style="text-align: center;">ACKNOWLEDGEMENT NO.</th>
        ...
        <tbody>
            @foreach ($deploy as $incoming_item)
            @php
            $personnel = App\Models\personnels::find($incoming_item->personnel_id);
            $incoming = App\Models\incoming::find($incoming_item->incoming_id);
            @endphp
            <tr class="table-light">
                <td scope="row" style="text-align: center;">{{$incoming_item->acknowledgement_no}}</td>
                <td scope="row" style="text-align: center;">{{$personnel->personnel_name}}</td>
                <td scope="row" style="text-align: center;">{{$personnel->position}}</td>
                <td scope="row" style="text-align: center;">{{$personnel->department}}</td>
                <td scope="row" style="text-align: center;">{{$personnel->office}}</td>
                <td scope="row" style="text-align: center;">{{$incoming->description}}</td>
                <td scope="row" style="text-align: center;">{{$incoming->serial_no}}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
